...
import xml.parsers.expat, os, errno, time, sys, operator, mavcrc
...
class MAVType(object):
    def __init__(self, name, id, linenumber, description=''):
        ...
        self.name_module = self.mk_name_module()
        self.name_camel = self.name_module[0].lower() + self.name_module[1:]
    ...
    def mk_name_module(self):
        def underscore_to_modulecase(value):
            def modulecase(): 
        ...
        lower_ascii = self.name_lower.encode('ascii','ignore')
        return underscore_to_modulecase(lower_ascii)
